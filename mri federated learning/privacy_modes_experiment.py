...
class ResearchClient(fl.client.NumPyClient):

```
def __init__(self, indices, dp_mode="none"):
...
def fit(self, parameters, config):
    ...
    if self.dp_mode == "standard":
        ...
        params = [p + np.random.normal(0, noise, p.shape) for p in params]


    elif self.dp_mode == "adaptive" and round_num > 5:

        sens = 0.00005 / (1 + np.sqrt(round_num))
...
for mode in ["none","standard","adaptive"]:
...
history = fl.simulation.start_simulation(
    ...
    client_fn=lambda cid:
        ResearchClient(hospital_indices[int(cid)], dp_mode=mode).to_client(),
